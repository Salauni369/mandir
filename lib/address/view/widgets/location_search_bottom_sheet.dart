...
// import 'package:zamboree_devotion/location/services/places_api_service.dart';
...
//   //
//   //   try {
//   //     final details = await widget.placesService.getPlaceDetails(placeId: suggestion.placeId);
...
//       controller.currentLatLng.value = LatLng(details.latitude, details.longitude);
//
//       // ✅ FIX: Use the returned Map to update title and address
//       final formatted = controller.formatTitleAndAddress(details.name, details.formattedAddress);
//       controller.locationTitle.value = formatted['title'] ?? '';
//       controller.locationAddress.value = formatted['address'] ?? '';
//
//       // Update distance
//       await controller.updateDistanceFromUser();
//
...
class _LocationSearchBottomSheetState extends State<LocationSearchBottomSheet> {
  ...
  Future<void> _fetchSuggestions(String query) async {
    ...
    try {
      ...
      print("Search suggestions: $results");
      _suggestions.assignAll(results);
    } catch (e) {
  ...
  Future<void> _onSuggestionTap(LocationSuggestion suggestion) async {
    ...
    try {
      print("🔍 Getting place details for: ${suggestion.placeId}");

      final details = await widget.placesService.getPlaceDetails(placeId: suggestion.placeId);

      print("✅ Place details received: ${details.name}, ${details.formattedAddress}");
      ...
      if (controller.circleList.isNotEmpty) {
        controller.circleList[0]['center'] = LatLng(details.latitude, details.longitude);
      }

      // ✅ FIX: Use the returned Map to update title and address
      final formatted = controller.formatTitleAndAddress(details.name, details.formattedAddress);
      controller.locationTitle.value = formatted['title'] ?? 'Selected Location';
      controller.locationAddress.value = formatted['address'] ?? details.formattedAddress;

      print("📍 Updated title: ${controller.locationTitle.value}");
      print("📮 Updated address: ${controller.locationAddress.value}");
